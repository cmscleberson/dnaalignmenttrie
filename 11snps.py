...
#gera as sequências mais encontradas em cada grupo de sites
    ...
    def compara(raiz, listPred, listAminos, listEsp):
        ...
        for j in range(len(listAminos)):
            ...
            if listAminAlt:
                print('Seq. Modificada: ', listAminAlt)
                print('Alterações na sequência de aminoácidos: ', alterAmino)

                print('Especies: ', listEspAlt)
        ...
        print('Espécies com alterações', alterSites)
        alterSites = 0
        print('Especies conservadas: ', listEspCons)
...
comp = root.compara(predominantes[0], conservados[0], especiesList)
...
#for m in range(len(conservados)):
    #comp = root.compara(conservados[m], predominantes[m], especiesList)
    #print(comp[2])
...
with open("saida.txt", "w") as arq:
    ...
    for j in range(len(indices)) :
        ...
        comp = root.compara(conservados[j], predominantes[j], especiesList)

        arq.write(str(comp[2]))
        arq.write('\n')

        arq.write('\nEspécies com alterações:\n')
        arq.write(str(comp[0]))
